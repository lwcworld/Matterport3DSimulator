...
plt.ion(); fig = plt.figure(); ax = fig.add_subplot(111); fig.canvas.draw()
...
if __name__ == "__main__":
    ...
    mapname = ['17DRP5sb8fy','1LXtFkjw3qL','1pXnuDYAj8r','29hnd4uzFmX','2azQ1b91cZZ','2n8kARJN3HM','2t7WUuJeko7',
    ...
    sim = sim_init(param_sim, [mapname[0]], random_initpos=True)
    while True:
        # move
        sim.makeAction([location], [heading], [elevation])
        ...
        if param_sim['randomwalk'] == False:
        ...
        elif param_sim['randomwalk']==True and param_sim['onlyneighbor']==True:
            if k == ord('q'):
                break
            ...
            ixs_open_local = [dict_ix_local[ix_global] for ix_global in ixs_open_global if dict_ix_local[ix_global]!=None]
            location = sample(ixs_open_local,1)[0]

        elif param_sim['randomwalk']==True and param_sim['onlyneighbor']==False:
            if k == ord('q'):
                break
            ...
            if i_journey == 0:
                ...
                ixs_notopen = [node for node, attr in G.nodes(data=True) if attr['isopen'] == False]\
                              + [ix_goal]
